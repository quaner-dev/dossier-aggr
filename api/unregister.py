...
from schemas import ResponseStatus, UnRegisterSchema, ResponseStatusList
...
async def unregister(data: UnRegisterSchema, service: APSService = Depends(APSService)):
    """注销接口"""
    device_id = data.UnRegisterObject.DeviceID
    aps = await service.query(device_id)
    await service.update_is_online(aps, StatusTypeEnum.Offline)
