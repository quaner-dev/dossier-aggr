...
**Validation:** Update the HTTP contract test so query params no longer filter results, then run targeted `pytest` commands and syntax compilation. Update `.ai` to reflect that `GET /VIID/Persons` returns the default top list instead of supporting query filters.
