...
**Architecture:** Keep runtime behavior unchanged in this slice. Tighten the protocol guard tests first so they fail against the outdated documentation, then update `.docs/PROTOCOL_2350.md`, `.docs/ARCHITECTURE.md`, and `.docs/TESTING.md` to describe the official PDF baseline and the currently known implementation gaps.
...
Expected: FAIL because `.docs/PROTOCOL_2350.md` still points to the deleted draft docx and does not describe the official-baseline gaps.
...
```bash
git add tests/protocol/test_protocol_matrix_guard.py .docs/PROTOCOL_2350.md
