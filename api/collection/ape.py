...
async def list_apes(service: Annotated[APEService, Depends(APEService)]):
    """查询所有APE信息"""
...
async def apes_create(
    ...
    _ = await service.batch_create_apes(apes)
...
async def apes_update(
    ...
    apes = data.APEListObject.APEObject
    for ape in apes:
        _ = await service.update_ape(ape.ApeID, ape)
...
async def apes_delete(
    ...
    for ape_id in ape_ids:
        _ = await service.delete_ape(ape_id)
